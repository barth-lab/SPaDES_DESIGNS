...
)
{
	...
	// LSPR
	// Need option in case no protein present (e.g. solo ion)
	if ( ratio >= 0.7 || option[ hydrate::no_inside_check_for_water_rotamers ]() )  { // If more than 70% of solid angle sections are blocked, xyz is considered inside the protein
		return true;
	} else {
		return false;
	}
		...
		if ( hydrate_V[ii] == true ) {
			...
			if ( temp_task->being_designed(ii) ) {
				...
				for ( core::Size jj=1; jj<= core::Size(rsd.natoms()); ++jj ) {
						...
						tp3->set_xyz( "O", rsd.xyz(jj) + 0.00001 ); // for neighbor calculation
						pose.append_residue_by_jump( *tp3, 1 );
						core::Size const pos1( pose.total_residue() );
						...
						(*water_info)[ pos1 ].design_anchor_index( 0 ); // not design, assign 0   wym
					...
					else if ( !rsd.is_protein() && !rsd.atom_type(jj).is_virtual() ) { // jklai4
                                                Size number_of_potential_waters = 0;
                                                bool rsd_is_ion = ( rsd.name() == "NA" );
                                                if ( rsd.atom_type(jj).is_acceptor() || rsd.atom_type(jj).is_polar_hydrogen() ) {
                                                        number_of_potential_waters = 1;
                                                }
                                                else if ( rsd_is_ion ) {
                                                        number_of_potential_waters = option[ OptionKeys::hydrate::waters_per_ion ];
                                                }
                                                else {
                                                        continue;
                                                }
                                                if ( !option[ OptionKeys::hydrate::attempt_all_polar ]() && !atom_is_hydratable(pose,ii,jj) ) continue;
                                                Size anchor_atom = jj;
                                                for ( Size potential_water=1; potential_water<=number_of_potential_waters; ++potential_water ){ // jklai4
                                                        if ( rsd_is_ion )
                                                                tp3->set_xyz( "O", rsd.xyz(jj) + 0.00001 ); // for neighbor calculation
                                                        else
                                                                tp3->set_xyz( "O", rsd.xyz(jj) ); // for neighbor calculation (original)
                                                        pose.append_residue_by_jump( *tp3, 1 );
...
get_ready_for_sew_packing(
	...
	for ( core::Size ii=1; ii<=pose.total_residue(); ++ii ) {
		if ( pose.residue(ii).name() == "TP3" && pose.energies().residue_total_energy(ii) == 0
				&& (*water_info)[ii].anchor_atom() != "NONE" ) {
			task->nonconst_residue_task( ii ).restrict_to_repacking();
			if ( !pose.residue( (*water_info)[ii].anchor_residue() ).is_protein() || pose.residue( (*water_info)[ii].anchor_residue()).name() == "X00" ) { // TO LSPR, THIS IS HARD CODED FOR X00 residue
				tp3->set_xyz( "O", pose.residue( (*water_info)[ii].anchor_residue()).xyz(1) + 0.00001 );
			}
			else {
			    tp3->set_xyz( "O", pose.residue((*water_info)[ii].anchor_residue()).xyz( "CA" ) ); // original
			}
			pose.replace_residue(ii,*tp3,false);
